...
const sections = [
  {
    ...
    subtitle: "Converting AI ambition into boardroom accountability",
    questions: [
      "Each AI initiative has a clear business owner.",
      "AI initiatives have written ROI hypotheses.",
      "ROI is linked to P&L-level KPIs.",
      "AI investments are prioritized by economic impact.",
      "AI portfolio visibility exists at executive level.",
      "AI impact is reported to the board.",
      "AI is integrated into strategy process.",
      "AI competitive advantage role is defined.",
      "Post-investment reviews are systematic.",
      "Stop/Go criteria are defined for initiatives.",
      "AI impact reflected in budgeting & forecasting.",
    ],
  },
  {
    id: "velocity",
    ...
    subtitle: "Speed and clarity from insight to action",
    questions: [
      "Critical data is rapidly available to leadership.",
      "Decision latency is measured.",
      "Decision ownership is clearly defined.",
      "Approval layers are minimized.",
      "AI insights directly inform decisions.",
      "Data reliability issues are rare.",
      "Cross-functional decision governance is clear.",
      "Decision-to-action ownership is explicit.",
      "Decision-to-implementation time is measured.",
      "Decision chain is visualized.",
      "Bottlenecks are systematically identified.",
    ],
  },
  {
    id: "execution",
    ...
    subtitle: "Turning AI outputs into operational reality",
    questions: [
      "AI KPIs cascade to team level.",
      "Incentives align with AI outcomes.",
      "Processes redesigned when AI introduced.",
      "AI integrated into daily management.",
      "AI usage rate is measured.",
      "User adoption tracked systematically.",
      "Capability gaps analyzed.",
      "Role-specific training implemented.",
      "AI operational impact documented.",
  ...
  },
  {
    id: "sales",
    label: "IV",
    title: "Sales & Value Realization",
    subtitle: "Translating intelligence into revenue and margin",
    questions: [
      "AI used for pipeline prioritization.",
      "AI improved win rate.",
      "Sales cycle shortened.",
      "Pricing uses data intelligence.",
      "AI-assisted segmentation active.",
      "Offer automation increased efficiency.",
      "Margin leakage measured.",
      "Churn analyzed predictively.",
      "Real-time sales visibility exists.",
      "AI impact on margin measured.",
      "AI used for upsell/cross-sell optimization.",
    ],
  },
  {
    id: "governance",
    ...
    subtitle: "Structural safeguards for sustainable AI value",
    questions: [
      "Executive AI sponsor appointed.",
      "Business & IT roles clearly separated.",
      "AI governance model documented.",
      "Board-level AI review cadence exists.",
      "AI risks assessed systematically.",
      "Data ownership clearly defined.",
      "Investment approval standardized.",
      "Ethical AI principles documented.",
      "Accountability clear in failures.",
      "AI roadmap linked to strategy roadmap.",
      "AI governance audit conducted within 12 months.",
    ],
  },
];
...
const aiStatuses = [
  { id: "exploring", label: "Tutkimme AI:n mahdollisuuksia" },
  { id: "piloting", label: "Meillä on käynnissä AI-pilotteja" },
  { id: "scaling", label: "Skalaamme AI-ratkaisuja" },
  { id: "mature", label: "AI on osa liiketoimintaamme" },
];
  ...
  >
    <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
      <span style={{
        ...
        border: `2px solid ${selected ? C.blue2 : C.grayDim}`,
        background: selected ? C.blue2 : "transparent",
...
export default function Home() {
  ...
  const tr = (fn) => {
    setAnimIn(false);
    setTimeout(() => { fn(); setAnimIn(true); }, 320);
  };
  ...
  const getSectionScore = (sId, questions) => questions.reduce((sum, _, i) => sum + (scores[`${sId}_${i}`] ?? 0), 0);
  ...
  const maturity = getMaturityLevel(scorePct);
  const exposure = calcExposure(profile, scorePct);
  const weakest = [...sections].sort((a, b) =>
    getSectionScore(a.id, a.questions) / a.questions.length -
    getSectionScore(b.id, b.questions) / b.questions.length
  )[0];
  const radarData = sections.map((s) => ({
    subject: s.title.split(" ")[0],
    score: Math.round((getSectionScore(s.id, s.questions) / (s.questions.length * 5)) * 100),
  }));
  ...
  const selectedStatus = aiStatuses.find((s) => s.id === profile.aiStatus);

  const inputStyle = {
    width: "100%", background: "rgba(255,255,255,0.04)",
    border: `1px solid ${C.grayDark}`, borderRadius: 10,
    padding: "13px 16px", color: C.white, fontSize: 15,
    marginBottom: 14, outline: "none", transition: "border-color 0.2s",
  ...
  return (
    <>
      <Head>
        <title>AI Execution Audit™</title>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-primary {
          ...
          letter-spacing: 0.05em; font-size: 16px; cursor: pointer;
          transition: opacity 0.2s, transform 0.1s;
        }
        .btn-primary:hover { opacity: 0.9; transform: translateY(-1px); }
        .btn-primary:disabled { background: ${C.grayDark}; color: ${C.grayDim}; cursor: default; transform: none; opacity: 1; }
        .btn-ghost {
          background: transparent; border: 1px solid ${C.grayDark}; border-radius: 10px;
          color: ${C.gray}; font-family: 'Barlow', sans-serif; font-size: 14px;
        ...
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 22px; height: 22px; border-radius: 50%; cursor: pointer; transition: box-shadow 0.2s; }
        ::-webkit-scrollbar { width: 4px; }
        ::-webkit-scrollbar-track { background: ${C.navyMid}; }
        ::-webkit-scrollbar-thumb { background: ${C.grayDark}; border-radius: 2px; }
      `}</style>
            ...
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: "40px 20px" }}>
              <div style={{ maxWidth: 540, width: "100%" }}>
                ...
                <div className="card glow" style={{ padding: 32 }}>
                  {!emailSent ? (
                    <>
                      <label className="bar" style={{ display: "block", fontSize: 12, color: C.gray, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>
                        Työsähköpostiosoite
                      </label>
                      <input
                        type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                        placeholder="sinä@yritys.fi" style={inputStyle}
                        onFocus={(e) => (e.target.style.borderColor = C.blue2)}
                        onBlur={(e) => (e.target.style.borderColor = C.grayDark)}
                      />
                      <button
                      ...
                      <p className="bar" style={{ fontSize: 12, color: C.grayDim, textAlign: "center", marginTop: 12 }}>
                        Ei salasanaa. Ei roskapostia. Linkki voimassa 24h.
                      </p>
                    </>
                  ) : (
                    <div style={{ textAlign: "center", padding: "12px 0" }}>
                      ...
                      <p className="bar" style={{ color: C.gray, fontSize: 14 }}>Linkki lähetetty osoitteeseen <span style={{ color: C.orange }}>{email}</span></p>
          ...
          {phase === "profile" && (
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ maxWidth: 680, margin: "0 auto", padding: "50px 20px 80px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 44 }}>
                ...
                <div style={{ marginLeft: "auto" }}>
                  <div style={{ display: "flex", gap: 5 }}>
                    {["P", ...sections.map((s) => s.label), "R"].map((l, i) => (
                      <div key={l} style={{ height: 4, width: i === 0 ? 28 : 8, borderRadius: 2, background: i === 0 ? C.orange : C.grayDark, transition: "all 0.4s" }} />
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 32 }}>
                <h2 className="barc" style={{ fontSize: "clamp(22px, 4vw, 32px)", fontWeight: 900, color: C.white, textTransform: "uppercase", marginBottom: 8 }}>
                  Kerro organisaatiostasi
                </h2>
                <p className="bar" style={{ color: C.gray, fontSize: 14, fontWeight: 300 }}>
                  Nämä tiedot kalibroivat tuloksesi ja taloudellisen altistuman laskennan.
                </p>
              </div>
              ...
              <div className="card" style={{ padding: 24, marginBottom: 16 }}>
                <p className="barc" style={{ fontSize: 16, fontWeight: 700, color: C.blue3, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
                  <span style={{ color: C.grayDim, marginRight: 8 }}>01</span> Roolisi organisaatiossa
                </p>
                <div className="opt-grid">
                  {roles.map((r) => <OptionBtn key={r.id} label={r.label} selected={profile.role === r.id} onClick={() => setProfile((p) => ({ ...p, role: r.id }))} />)}
                </div>
              </div>

              <div className="card" style={{ padding: 24, marginBottom: 16 }}>
                <p className="barc" style={{ fontSize: 16, fontWeight: 700, color: C.blue3, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
                  <span style={{ color: C.grayDim, marginRight: 8 }}>02</span> Organisaation koko
                </p>
                <div className="opt-grid">
                  {sizes.map((s) => <OptionBtn key={s.id} label={s.label} selected={profile.size === s.id} onClick={() => setProfile((p) => ({ ...p, size: s.id }))} />)}
                </div>
              </div>

              <div className="card" style={{ padding: 24, marginBottom: 16 }}>
                <p className="barc" style={{ fontSize: 16, fontWeight: 700, color: C.blue3, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
                  <span style={{ color: C.grayDim, marginRight: 8 }}>03</span> Toimiala
                </p>
                <div className="opt-grid">
                  {industries.map((i) => <OptionBtn key={i.id} label={i.label} selected={profile.industry === i.id} onClick={() => setProfile((p) => ({ ...p, industry: i.id }))} />)}
                </div>
              </div>

              <div className="card" style={{ padding: 24, marginBottom: 32 }}>
                <p className="barc" style={{ fontSize: 16, fontWeight: 700, color: C.blue3, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>
                  <span style={{ color: C.grayDim, marginRight: 8 }}>04</span> AI-kypsyystaso tällä hetkellä
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {aiStatuses.map((s) => <OptionBtn key={s.id} label={s.label} selected={profile.aiStatus === s.id} onClick={() => setProfile((p) => ({ ...p, aiStatus: s.id }))} />)}
                </div>
              </div>

              <button className="btn-primary" style={{ width: "100%", padding: "16px" }} disabled={!profileComplete} onClick={() => tr(() => setPhase("survey"))}>
                ALOITA ARVIOINTI →
              </button>
            </div>
          )}
          ...
          {phase === "survey" && (
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ maxWidth: 740, margin: "0 auto", padding: "40px 20px 80px" }}>
              ...
              <div style={{ marginBottom: 36 }}>
                ...
                <h2 className="barc" style={{ fontSize: "clamp(22px, 4vw, 36px)", fontWeight: 900, color: C.white, textTransform: "uppercase", marginBottom: 6 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {currentSection.questions.map((q, i) => {
                  ...
                  const hasVal = val >= 0;
                  const trackBg = hasVal ? `linear-gradient(90deg, ${scoreColors[val]} ${val * 20}%, ${C.grayDark} ${val * 20}%)` : C.grayDark;
                  return (
                    <div key={i} className="card" style={{ padding: "22px 24px", borderColor: hasVal ? "rgba(30,136,229,0.2)" : "rgba(255,255,255,0.06)", transition: "border-color 0.3s" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 18 }}>
                        <p className="bar" style={{ fontSize: 14, color: C.gray, lineHeight: 1.6 }}>
                          <span style={{ color: C.grayDim, fontFamily: "monospace", fontSize: 11, marginRight: 8 }}>{String(i + 1).padStart(2, "0")}</span>
                          {q}
                        </p>
                      ...
                      <input
                        type="range" min={0} max={5} step={1} value={hasVal ? val : 0}
                        style={{ background: trackBg }}
                        onChange={(e) => setScores((prev) => ({ ...prev, [key]: Number(e.target.value) }))}
                        onClick={(e) => setScores((prev) => ({ ...prev, [key]: Number(e.target.value) }))}
                      />
                      <style>{`
                        input[type=range] { background: ${trackBg}; }
                        input[type=range]::-webkit-slider-thumb { background: ${hasVal ? scoreColors[val] : C.grayDim}; box-shadow: 0 0 0 3px ${hasVal ? scoreColors[val] + "30" : "transparent"}; }
                      `}</style>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 36 }}>
                <button className="btn-ghost" style={{ padding: "12px 22px" }} disabled={sectionIndex === 0} onClick={() => tr(() => setSectionIndex((i) => i - 1))}>
                  ← Takaisin
                </button>
                <span className="bar" style={{ fontSize: 12, color: C.grayDim }}>
                  {currentSection.questions.filter((_, i) => scores[`${currentSection.id}_${i}`] !== undefined).length} / {currentSection.questions.length} vastattu
                </span>
                <button
                  className="btn-primary" style={{ padding: "12px 28px" }}
                  disabled={!sectionDone(currentSection)}
          ...
          {phase === "results" && (
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ maxWidth: 900, margin: "0 auto", padding: "52px 20px 80px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 48, flexWrap: "wrap", gap: 16 }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <p className="bar" style={{ fontSize: 12, color: C.grayDim }}>
                    {selectedRole?.label} · {selectedSize?.label}<br />{selectedIndustry?.label} · {selectedStatus?.label}
                  </p>
                </div>
              </div>
                ...
                <div className="card-blue glow" style={{ padding: "36px 24px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ position: "relative", marginBottom: 20 }}>
                    <svg width="160" height="160" viewBox="0 0 160 160">
                      ...
                      <defs>
                        <linearGradient id="grad_score" x1="0" y1="0" x2="1" y2="0">
                          <stop offset="0%" stopColor={C.blue1} />
                          <stop offset="100%" stopColor={C.blue3} />
                        </linearGradient>
                      </defs>
                    </svg>
                      ...
                      <div key={s.id}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            {isWeakest && (
                              <span className="bar" style={{ fontSize: 10, background: "rgba(239,68,68,0.1)", color: "#ef4444", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 100, padding: "2px 10px", fontWeight: 600 }}>
                                ⚠ Kriittinen
                              </span>
                            )}
                          </div>
                        ...
                        <div style={{ height: 6, borderRadius: 3, background: C.grayDark, overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${pct}%`, background: `linear-gradient(90deg, ${barColor}, ${barColor}cc)`, borderRadius: 3, boxShadow: `0 0 8px ${barColor}50` }} />
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="two-col">
                <div className="card" style={{ padding: "28px", background: "rgba(239,68,68,0.04)", borderColor: "rgba(239,68,68,0.15)" }}>
                  ...
                  <p className="bar" style={{ fontSize: 13, color: C.gray, lineHeight: 1.7 }}>
                    Vuosittainen arvo on vaarassa toteutumattomien AI-hyötyjen takia. Laskelma perustuu profiiliisi: {selectedSize?.label}, {selectedIndustry?.label}.
                  </p>
                  <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid rgba(255,255,255,0.05)` }}>
                    <p className="bar" style={{ fontSize: 12, color: C.grayDim, marginBottom: 4 }}>Kriittisin kehityskohde:</p>
                    <p className="barc" style={{ fontSize: 16, fontWeight: 800, color: "#ef4444", textTransform: "uppercase", letterSpacing: "0.04em" }}>{weakest.title}</p>
                  </div>
                </div>
                ...
                <div className="card" style={{ padding: "28px", background: "rgba(245,124,0,0.06)", borderColor: "rgba(245,124,0,0.2)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <p className="barc" style={{ fontSize: 20, fontWeight: 800, color: C.white, marginBottom: 12, lineHeight: 1.3, textTransform: "uppercase", letterSpacing: "0.02em" }}>Valmis sulkemaan suorituskykyvajeen?</p>
                    <p className="bar" style={{ fontSize: 13, color: C.gray, lineHeight: 1.7 }}>
                      45 minuutin strategiasessio. Kartoitamme yhdessä suurimman vipuvaikutuksen interventiokohdat ja rakennamme 90 päivän toteutusroadmapin.
                    </p>
                  </div>
                  <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 10 }}>
                    <button className="btn-primary" style={{ padding: "14px", width: "100%", fontSize: 15 }}>
                      VARAA STRATEGIASESSIO →
                    </button>
                    <button className="btn-ghost" style={{ padding: "12px", width: "100%", fontSize: 13 }}>
                      Lataa PDF-raportti
                    </button>
                  </div>
                </div>
              ...
              <div style={{ textAlign: "center", marginTop: 48 }}>
                <Logo size={28} />
                <p className="bar" style={{ fontSize: 11, color: C.grayDim, marginTop: 12, lineHeight: 1.8 }}>
                  AI Execution Audit™ · 55 kysymystä · 5 dimensiota
                </p>
                <button
                  onClick={() => tr(() => { setPhase("landing"); setScores({}); setSectionIndex(0); setEmailSent(false); setProfile({ role: "", size: "", industry: "", aiStatus: "" }); })}
