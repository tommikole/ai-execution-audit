...
export default function App() {
  ...
  const handleSendMagicLink = async () => {
    ...
    if (error) {
      setEmailError("Virhe lähetyksessä. Tarkista sähköpostiosoite.");
    } else {
      setEmailSent(true);
    }
  };
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 22px; height: 22px; border-radius: 50%; cursor: pointer; background: ${C.blue2}; }
        ...
        .btn-primary:disabled { background: ${C.grayDark}; color: ${C.grayDim}; cursor: default; }
        .btn-ghost { background: transparent; border: 1px solid ${C.grayDark}; border-radius: 10px; color: ${C.gray}; font-family: 'Barlow', sans-serif; font-size: 14px; cursor: pointer; }
        .btn-ghost:disabled { opacity: 0.3; cursor: default; }
        .spinner { width: 18px; height: 18px; border: 2px solid rgba(255,255,255,0.2); border-top-color: #fff; border-radius: 50%; animation: spin 0.7s linear infinite; display: inline-block; margin-right: 8px; vertical-align: middle; }
                  ...
                  {!emailSent ? (
                    <>
                      ...
                      <input
                        ...
                        placeholder="sinä@yritys.fi" className="bar"
                        style={{ width: "100%", background: "rgba(255,255,255,0.04)", border: `1px solid ${emailError ? "#ef4444" : C.grayDark}`, borderRadius: 10, padding: "13px 16px", color: C.white, fontSize: 15, marginBottom: 14, outline: "none", transition: "border-color 0.2s" }}
                        onFocus={e => e.target.style.borderColor = C.blue2}
                        onBlur={e => e.target.style.borderColor = emailError ? "#ef4444" : C.grayDark}
                      />
                      {emailError && <p className="bar" style={{ fontSize: 13, color: "#ef4444", marginBottom: 12 }}>{emailError}</p>}
                      <button className="btn-primary" style={{ width: "100%", padding: "14px" }}
                        disabled={!email.includes("@") || emailLoading}
                        onClick={handleSendMagicLink}>
                        {emailLoading ? <><span className="spinner" />LÄHETETÄÄN...</> : "LÄHETÄ PÄÄSYLINKKI →"}
                      </button>
                    ...
                    <div style={{ textAlign: "center", padding: "12px 0" }}>
                      ...
                      <p className="bar" style={{ color: C.gray, fontSize: 14 }}>Pääsylinkki lähetetty osoitteeseen <span style={{ color: C.orange }}>{email}</span></p>
                      <p className="bar" style={{ color: C.grayDim, fontSize: 13, marginTop: 12, lineHeight: 1.6 }}>Klikkaa sähköpostissa olevaa linkkiä — sinut ohjataan suoraan arviointiin.</p>
                      <button onClick={() => setEmailSent(false)} className="bar" style={{ marginTop: 16, background: "transparent", border: "none", color: C.grayDim, fontSize: 13, cursor: "pointer", textDecoration: "underline" }}>
                        Väärä osoite? Yritä uudelleen
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ maxWidth: 680, margin: "0 auto", padding: "50px 20px 80px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 44 }}>
                ...
                <div>
                  ...
                  <div className="barc" style={{ fontSize: 18, color: C.orange, fontWeight: 800, textTransform: "uppercase" }}>Taustaprofiili</div>
                </div>
                {user && <p className="bar" style={{ marginLeft: "auto", fontSize: 12, color: C.grayDim }}>✓ {user.email}</p>}
              </div>
            ...
            <div className={`fade ${animIn ? "fade-in" : "fade-out"}`} style={{ maxWidth: 900, margin: "0 auto", padding: "52px 20px 80px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 48, flexWrap: "wrap", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    <div className="barc" style={{ fontSize: 11, color: C.blue3, letterSpacing: "0.15em", textTransform: "uppercase" }}>AI™ Execution Audit</div>
                    <div className="barc" style={{ fontSize: 22, fontWeight: 900, color: C.orange, textTransform: "uppercase" }}>Arviointiraportti</div>
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <p className="bar" style={{ fontSize: 12, color: C.grayDim }}>{selectedRole?.label} · {selectedSize?.label}<br />{selectedIndustry?.label}</p>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }} className="two-col">
                <div className="card-blue glow" style={{ padding: "36px 24px", textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ padding: "8px 22px", background: maturity.color + "20", border: `1px solid ${maturity.color}50`, borderRadius: 100, marginBottom: 12 }}>
                    <span className="barc" style={{ fontSize: 16, fontWeight: 800, color: maturity.color }}>{maturity.label}</span>
                  </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }} className="two-col">
                <div className="card" style={{ padding: "28px", background: "rgba(239,68,68,0.04)", borderColor: "rgba(239,68,68,0.15)" }}>
                  ...
                  <p className="barc" style={{ fontSize: "clamp(24px, 3.5vw, 38px)", fontWeight: 900, color: "#ef4444", marginBottom: 6 }}>€{exposure.toLocaleString("fi-FI")}</p>
                  <p className="bar" style={{ fontSize: 13, color: C.gray, lineHeight: 1.7 }}>Vuosittainen arvonlisäys on vaarassa toteutumattomien AI-hyötyjen takia. Laskelma perustuu profiiliisi ({selectedSize?.label}, {selectedIndustry?.label}).</p>
                ...
                <div className="card" style={{ padding: "28px", background: "rgba(245,124,0,0.06)", borderColor: "rgba(245,124,0,0.2)", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    ...
                    <p className="bar" style={{ fontSize: 13, color: C.gray, lineHeight: 1.7 }}>45 minuutin strategiasessio. Kartoitamme suurimman vipuvaikutuksen interventiokohdat ja rakennamme 90 päivän toteutusroadmapin.</p>
                  </div>
              ...
              <div style={{ textAlign: "center", marginTop: 48 }}>
                ...
                <button onClick={async () => { await supabase.auth.signOut(); tr(() => { setPhase("landing"); setScores({}); setSectionIndex(0); setEmailSent(false); setEmail(""); setProfile({ role: "", size: "", industry: "", aiStatus: "" }); }); }}
                  ...
                  Kirjaudu ulos / Tee arviointi uudelleen
                </button>
              </div>
            </div>
          )}
